...
import com.example.artilleryandroid.sdkDemo.Offline.*;
import java.util.ArrayList;
import java.util.Locale;

public class OfflineMapDemoList extends Activity implements OnClickListener, OnGetMapsResult, OnDownLoadResult, OnChildClickListener,OnMapParamChangeListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.example.artilleryandroid.R.layout.offlinemapdemolist);
		...
		// 设置离线地图文件存放的根目录
		String path = "/sdcard/tianditu/staticmap/";
		offlineMapMng.setMapPath(path);
	//	offlineMapMng.setMapPath("/sdcard/tianditu3/staticmap/");


		TextView textPath = (TextView) findViewById(com.example.artilleryandroid.R.id.text_path);
		textPath.setText("离线地图路径：" + path);
		
		// 城市列表
		mBtnAllMaps = (Button) findViewById(com.example.artilleryandroid.R.id.btn_allmaps);
		mBtnAllMaps.setOnClickListener(this);
		// 下载中
		mBtnDownLoading = (Button) findViewById(com.example.artilleryandroid.R.id.btn_downloading);
		mBtnDownLoading.setOnClickListener(this);
		// 已下载列表
		mBtnDownLoaded = (Button) findViewById(com.example.artilleryandroid.R.id.btn_downloaded);
		mBtnDownLoaded.setOnClickListener(this);
		// 地图视图
		mBtnMap = (Button) findViewById(com.example.artilleryandroid.R.id.btn_map);
		mBtnMap.setOnClickListener(this);
		
		// 刷新城市列表
		Button btn = (Button) findViewById(com.example.artilleryandroid.R.id.btn_refresh);
		btn.setOnClickListener(this);
		// 扫描离线包
		btn = (Button) findViewById(com.example.artilleryandroid.R.id.btn_scan);
		btn.setOnClickListener(this);
		// 停止全部
		btn = (Button) findViewById(com.example.artilleryandroid.R.id.btn_stopall);
		btn.setOnClickListener(this);

		// 城市列表
		mAdapterAllMap = new AllMapAdapter(this);
		mListAllMaps = (ExpandableListView) findViewById(com.example.artilleryandroid.R.id.list_allmaps);
		...
		mListDownLoading = (ExpandableListView)findViewById(com.example.artilleryandroid.R.id.list_downloading);
		...
		mListDownLoaded = (ExpandableListView) findViewById(com.example.artilleryandroid.R.id.list_downloaded);
		mListDownLoaded.setAdapter(mAdapterDownLoaded);
		mListDownLoaded.setOnChildClickListener(this);
		// 搜索城市列表
		mEdit = (EditText)findViewById(com.example.artilleryandroid.R.id.edit_name);
		mEdit.setOnKeyListener(new OnKeyListener() {
			...
			public boolean onKey(View v, int keyCode, KeyEvent event) {
				...
				if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_UP) {
					...
					Toast.makeText(OfflineMapDemoList.this, str, Toast.LENGTH_SHORT).show();;
					return true;
				}
				return false;
			}
		...
		mViewMap = (View)findViewById(com.example.artilleryandroid.R.id.layout_map);
		mEditLon = (EditText)findViewById(com.example.artilleryandroid.R.id.btn_lon);
		mEditLat = (EditText)findViewById(com.example.artilleryandroid.R.id.btn_lat);
		mEditScale = (EditText)findViewById(com.example.artilleryandroid.R.id.btn_scale);
		mBtnMoveTo = (Button)findViewById(com.example.artilleryandroid.R.id.btn_moveto);
		mBtnMoveTo.setOnClickListener(this);

		// 地图视图
		mMapView = (MapView)findViewById(com.example.artilleryandroid.R.id.layout_mapview);
		// 设置地图缓冲区路径
	...
	public void onClick(View view) {
		switch (view.getId()) {
		case com.example.artilleryandroid.R.id.btn_allmaps:
			setViewPage(PAGE_ALLMAPS);
			break;
		case com.example.artilleryandroid.R.id.btn_downloading:
			setViewPage(PAGE_DOWNLOADING);
			break;
		case com.example.artilleryandroid.R.id.btn_downloaded:
			setViewPage(PAGE_DOWNLOADED);
			break;
		case com.example.artilleryandroid.R.id.btn_map:
			setViewPage(PAGE_MAPVIEW);
			break;
		case com.example.artilleryandroid.R.id.btn_refresh:
			mProgressDlg = new ProgressDialog(this);
			mProgressDlg.setTitle("天地图");
			mProgressDlg.setProgressStyle(ProgressDialog.STYLE_SPINNER);
			...
			break;
		case com.example.artilleryandroid.R.id.btn_scan: {
	...
	//		mMapView.setOfflineMaps(offlineMapMng.searchLocalMaps());
		...
		case com.example.artilleryandroid.R.id.btn_stopall:
			offlineMapMng.pauseDownload();
			break;
		case com.example.artilleryandroid.R.id.btn_moveto: {
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        case DIALOG_PAGE_ALLMAPS: {
			...
			return new AlertDialog.Builder(OfflineMapDemoList.this)
					.setTitle(title)
	...
	public void onMapParamChange(int iwitchchange) {
		...
		Log.v("onMapParamChange", "witch"+iwitchchange);
	}
}
