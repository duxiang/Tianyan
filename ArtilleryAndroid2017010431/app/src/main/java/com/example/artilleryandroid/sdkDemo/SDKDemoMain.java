...
public class SDKDemoMain extends ExpandableListActivity {
	...
	private static final String mPackage = "com.example.artilleryandroid.";
	...
	private List<SDKDemoGroup> getData(String prefix) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (prefix.length() == 0 || label.startsWith(prefix)) {
				...
				if (nextLabel != null && nextLabel.equals(getString(com.example.artilleryandroid.R.string.app_name)))
					nextLabel = nextClass;
	...
	class MyExpandableListAdapter extends BaseExpandableListAdapter {
		...
		public View getGroupView(int groupPosition, boolean isExpanded, View convertView,
				ViewGroup parent) {
			if (convertView == null) {
				int height = mContext.getResources().getDimensionPixelSize(com.example.artilleryandroid.R.dimen.groupheight);
				int padding = mContext.getResources().getDimensionPixelSize(com.example.artilleryandroid.R.dimen.margin);
